...
def render_pie_chart(launch_site):
    if launch_site == 'ALL':
        dfFiltrado = spacex_df.groupby('Launch Site').mean(numeric_only=True).reset_index()
        ...
        fig = px.pie(dfFiltrado,
                     values='count',
                     names='class',
                     title=f'Total count of Succes and Failed Landings for {launch_site}')
        return fig
# TASK 4:
